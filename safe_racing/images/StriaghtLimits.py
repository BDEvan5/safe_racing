...
def plot_imgs():
    ...
    ab = AnnotationBbox(oi, (1.3, -0.4), xycoords='data', frameon=False)
    a1.add_artist(ab)

    init_state = np.array([1.5, -0.4, 0])
    ...
    oi = OffsetImage(img, zoom=0.4)
    ab = AnnotationBbox(oi, (1.5, 0.), xycoords='data', frameon=False)
    a2.add_artist(ab)

    init_state = np.array([1.5, 0.2, np.deg2rad(angle)])
    left_poses, right_poses = make_cricles(init_state, max_steer)
    ...
    plt.tight_layout()
    plt.savefig("Imgs/straight_line_safety.svg")
    plt.savefig("Imgs/straight_line_safety.pdf")
